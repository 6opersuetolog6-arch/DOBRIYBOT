...
# === ID РОЛЕЙ И КАНАЛОВ ===
OWNER_ROLE_ID = 1470473505702281265  # ← ЗАМЕНИТЕ
DEP_OWNER_ROLE_ID = 1470473509015781416  # ← ЗАМЕНИТЕ
HIGH_RANG_ROLE_ID = 1470473522873634858  # ← ЗАМЕНИТЕ
RECRUIT_ROLE_ID = 1470473526556491861   # ← ЗАМЕНИТЕ
MAIN_ROLE_ID = 1470473528955371763      # ← ЗАМЕНИТЕ
NEWBIE_ROLE_ID = 1470473532327854173    # ← ЗАМЕНИТЕ
COMMON_ROLE_ID = 1470473534999494827    # ← ЗАМЕНИТЕ

THREADS_CHANNEL_ID = 1470473650338660352  # ← ЗАМЕНИТЕ
LOG_CHANNEL_ID = 1470473620336935034      # ← ЗАМЕНИТЕ
...
async def reset_all_cooldowns(interaction: discord.Interaction):
    if DEP_OWNER_ROLE_ID not in [role.id for role in interaction.user.roles]:
        try:
            ...
            await interaction.followup.send("❌ Эта команда доступна только Заместителю Владельца.", ephemeral=True)
        return
...
async def recruitment(interaction: discord.Interaction, channel_id: str):
    ...
    if not has_any_role(interaction.user, allowed_roles):
        try:
            ...
            await interaction.followup.send("❌ Эта команда доступна только Владельцу и Заместителю.", ephemeral=True)
        return
...
async def restore_backup(interaction: discord.Interaction, date: str):
    if OWNER_ROLE_ID not in [role.id for role in interaction.user.roles]:
        try:
            ...
            await interaction.followup.send("❌ Только Владелец может восстанавливать состав.", ephemeral=True)
        return
